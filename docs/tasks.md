...
- [~] (P1) (L) Build `KB_MainMenuVisual.js` (in-game pause menu, sumi-e ink-wash) — spec at `docs/spec/main-menu.md`
  ...
  - [x] Step 9: Header band + hint + actor-selection flow ✅ shipped 2026-05-26 (v0.5.0 through v0.6.20) — full-width 64px band, gameTitle (left); location + Thời Gian + gold/Lượng (right-aligned, repaints every second). Vertical ink-wash gradient + brushstroke hairline. New locale key `menu_label_playtime` added to vi/en. **v0.5.1 fix:** hides VisuMZ's bottom Time/Gold/Variable bars + moves Button Assist to bottom. **v0.6.6 fix**: right cluster padding now derives from `_cancelButton.x` dynamically. **v0.6.7**: custom centered hint (`[Z] Chọn   [X] Hồi`) now on every menu scene. **v0.6.8**: scene-aware hint entries. **v0.6.10**: `Scene_MenuBase.mainAreaHeight` capped above hint. **v0.6.9**: class subtitle styling on Scene_Skill + Scene_Equip. **v0.6.11–v0.6.13** (iterative fixes): Scene_Status General tab rendered by `VisuMZ_1_ElementStatusCore.Window_StatusData`; used drawTextEx interception to apply subtitle styling, resolved via KBLocalization. **v0.6.14**: class text now resolves `{classStudent}` → "Sinh Viên" before draw. **v0.6.15**: runtime-patched ElementStatusCore's DrawJS to expand `basicDataHeight`, moving DP gauge fully into the panel. **v0.6.3–v0.6.5** (actor-selection flow): Skill/Equip/Status/ClassChange commands activate stock status window invisibly; three aliases (`commandPersonal`, `onPersonalCancel`, `onPersonalOk`) manage visibility + party column toggle to prevent double-render. **v0.6.16 (Formation freeze fix)**: Formation (Đội Hình) uses separate `commandFormation` entry point; same three aliases now added for Formation (`commandFormation` + `onFormationCancel` alias, `onFormationOk` NOT aliased). `onFormationCancel` calls `_kbPartyColumn.refresh()` after restore so party order reflects any swap that occurred. **v0.6.17 (ghost frame fix)**: thin empty rectangle above the bottom hint on every menu scene — VisuMZ's button-assist windowskin frame was still rendering despite `visible = false`. Added `setBackgroundType(2)` in the assist-hide path; same `setBackgroundType(2)` trick used on the stock status window in v0.5.5. **v0.6.18 (ghost frame, continued)**: rectangle still visible after v0.6.17 — the leaking frame was actually one of the three VisuMZ MainMenuCore windows we suppress in the `HEADER_HIDE_BARS` block (`_playtimeWindow` / `_variableWindow` / `_goldWindow`). Same `setBackgroundType(2)` fix applied to all three within the loop. **v0.6.19 (ghost frame, iteration #3)**: rectangle still visible. Hypotheses: (1) timing — VisuMZ creates the assist window mid-create() AFTER our Scene_MenuBase.create alias runs, so our hide never reaches it; (2) refresh leak — update loop re-applies frame each tick. Belt-and-suspenders fix: factored hide logic into `_kbNukeAssist(scene)` helper that does `visible=false` + `deactivate()` + `setBackgroundType(2)` + `move(x, y, 0, 0)`. 0×0 dimensions guarantee nothing renders. Called from both Scene_MenuBase.create AND new Scene_MenuBase.start alias. start() fires after full create chain, so assist guaranteed to exist. **v0.6.20 (ghost frame, iteration #4)**: still visible. Applied the same `move(x, y, 0, 0)` dimension-nuke to the Playtime/Variable/Gold trio (v0.6.18 only got their backgroundType). Added a one-shot ghost-dump on Scene_MenuBase.start (gated on Debug Logging param): walks `_windowLayer.children` and logs any child whose bottom sits within 100 px of the hint top — class name + x/y/w/h + visible + _backgroundType. If the rectangle persists, enable Debug Logging in Plugin Manager and the console will name the culprit. **v0.6.21 (ghost frame, RESOLVED)**: debug dump from v0.6.20 nailed it — a raw `Window_Base` (no specific subclass) at `(240, 641) 536×60` leaking from some map-side popup plugin (CGMZ_MapNameWindow / IgnisItemGoldPopup / similar). After `_kbNukeAssist` in start(), iterate `_windowLayer.children` and suppress (visible=false + setBackgroundType(2) + 0×0) any child whose `constructor.name === 'Window_Base'` with bottom edge within 200 px above the hint. Safe because we never create raw `Window_Base` instances — always subclass.
