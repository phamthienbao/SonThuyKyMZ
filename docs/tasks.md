...
- [~] (P1) (L) Build `KB_MainMenuVisual.js` (in-game pause menu, sumi-e ink-wash) — spec at `docs/spec/main-menu.md`
  ...
  - [x] Step 9: Header band ✅ shipped 2026-05-26 (v0.5.0 + v0.5.1 + v0.6.6 + v0.6.7) — full-width 64px band, gameTitle (left); location + Thời Gian + gold/Lượng (right-aligned, repaints every second). Vertical ink-wash gradient + brushstroke hairline. New locale key `menu_label_playtime` added to vi/en. **v0.5.1 fix:** hides VisuMZ's bottom Time/Gold/Variable bars (now redundant) + moves VisuMZ's Button Assist window to the bottom (was overlapping the header). Gated by `Header Hides VisuMZ Bars` + `Header Relocates Button Assist` params (default both true). **User action**: flip `Enable Header` param to true in Plugin Manager. **v0.6.6 fix**: right cluster (`0 Lượng` / playtime / location) was sliding underneath the top-right cancel button; right padding now derives from `_cancelButton.x` dynamically with a 12-px gap. **v0.6.7**: custom centered hint (`[Z] Chọn   [X] Hồi`) now appears on every menu scene (Item/Skill/Equip/Status/Options/Save/Quest/Journal/etc.) via a new `Scene_MenuBase.create` alias. Replaces VisuMZ's right-aligned button-assist bar on all scenes, not just Scene_Menu.
