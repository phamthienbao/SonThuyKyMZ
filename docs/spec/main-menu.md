# Main Menu (KB_MainMenuVisual v0.6.21 shipped)
...
- **Status:** v0.6.21 shipped (iteration #5 / RESOLVED on ghost-frame issue: pinpointed the culprit via v0.6.20's debug dump — a raw `Window_Base` instance (not a named subclass) sitting at (240, 641) w=536 h=60, visible=true, no backgroundType. Likely a leftover popup from CGMZ_MapNameWindow or similar that doesn't tear down on scene transition. Fixed by walking `this._windowLayer.children` in Scene_MenuBase.start and applying `visible=false` + `setBackgroundType(2)` + `move(x, y, 0, 0)` to any child whose `constructor.name === 'Window_Base'` AND whose bottom edge sits within 200 px above the hint. Safe because KB never instantiates raw Window_Base — all windows are named subclasses. The v0.6.20 debug-dump code remains for future diagnostics. 5-iteration ghost-frame saga resolved: v0.6.17 (button assist) → v0.6.18 (Playtime/Var/Gold) → v0.6.19 (timing + dimensions) → v0.6.20 (debug instrumentation) → v0.6.21 (root-cause suppression)). v0.6.20 shift: applied `move(x, y, 0, 0)` to Playtime/Variable/Gold windows + one-shot debug dump gated on Debug Logging param. v0.6.19 shift from timing + refresh-leak issues: factored assist-hide into `_kbNukeAssist(scene)` helper with belt-and-suspenders — `visible=false` + `deactivate()` + `setBackgroundType(2)` + `move(0, 0, 0, 0)` (0×0 guarantee). Called from both Scene_MenuBase.create AND new Scene_MenuBase.start alias. v0.6.18 applied `setBackgroundType(2)` to three VisuMZ MainMenuCore windows in the `HEADER_HIDE_BARS` block (`_playtimeWindow`, `_variableWindow`, `_goldWindow`). v0.6.17 added `setBackgroundType(2)` to button-assist hide. v0.6.16 fixed Formation freeze via separate `commandFormation` alias. v0.6.15 runtime-patched ElementStatusCore's General-tab DrawJS to expand `basicDataHeight` from `6.5*lineHeight` to `7.5*lineHeight`, shifting actor info block up ~36px so DP fits. v0.6.14 resolved class subtitle localization via `KBLocalization.process()`. v0.6.13 discovered ElementStatusCore renders class via `drawTextEx` (not `drawText`); drawTextEx interception workaround. v0.6.10 capped `Scene_MenuBase.mainAreaHeight` at available space above hint. v0.6.9 added class subtitle styling and DP gauge height on Scene_Skill via `Window_StatusBase.drawActorClass` override + `Scene_Skill.statusWindowHeight` + `Window_SkillStatus.refresh` top-aligned drawing.
